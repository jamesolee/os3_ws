...
void StereoInertialNode::SyncWithImu()
{
    ...
    while (1)
    {
        ...
        if (!imgLeftBuf_.empty() && !imgRightBuf_.empty() && !imuBuf_.empty())
        {
            ...
            // Synchronise right image to left image
            bufMutexRight_.lock();
            ...
            // Just in case no right image was found that is synchronised with 
            // the first left image, synchronise left image with the last right image.
            bufMutexLeft_.lock();
            ...
            // If time diff between cameras is too large, skip
